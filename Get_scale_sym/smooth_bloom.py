...
def smooth_Llama_ln_fcs(ln, fcs, act_scales, alpha=0.5):
...
@torch.no_grad()
def smooth_bloomlm(model, scales, alpha=0.5):
    for name, module in model.named_modules():
        if isinstance(module, OPTDecoderLayer):
            ...
            smooth_Llama_ln_fcs(attn_ln, qkv, qkv_input_scales, alpha)

            ffn_ln = module.post_attention_layernorm
            fc1 = [module.mlp.gate_proj, module.mlp.up_proj]
            fc1_input_scales = torch.squeeze(scales[name + '.mlp.gate_proj']["input"])
            smooth_Llama_ln_fcs(ffn_ln, fc1, fc1_input_scales, alpha)